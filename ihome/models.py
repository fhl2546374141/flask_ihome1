...
class User(BaseModel, db.Model):
    ...
    name = db.Column(db.String(32), unique=True, nullable=False)  # 用户昵称  唯一约束 只能有一个
    password_hash = db.Column(db.String(128), nullable=False)  # 加密的密码
    mobile = db.Column(db.String(11), unique=True, nullable=False)  # 手机号  唯一约束 只能有一个
    ...
    # 密码加密
    # 加上property装饰器后，会把函数变为属性，函数名就是属性名
    @property  # @property 可以将一个方法的调用方式变成“属性调用”.@property==>相当于password属性的getter 方法
    def password(self):   # 读取
        """读取password属性时被调用"""
        # 表示密码不能读取 只能设置
        raise AttributeError("不可读")

    @password.setter
    def password(self, passwd):   # 相当于 password 属性的 setter 方法 设置
        """设置密码时被调用,设置密码加密"""
...
class House(BaseModel, db.Model):
    ...
    def to_full_dict(self):
        ...
        # 评论信息
        comments = []
        # orders = Order.query.filter(Order.house_id=self.id, Order.status=='COMPLETE', Order.comment != None).order_by(Order.update_time.desc()).limit(constants.HOUSE_DETAIL_REDIS_EXPIRE_SECOND)
        orders = Order.query.filter(Order.house_id == self.id, Order.status == 'COMPLETE', Order.comment != None).order_by(Order.update_time.desc()).limit(constants.HOUSE_DETAIL_REDIS_EXPIRE_SECOND)
        for order in orders:
            comment = {
                "comment": order.comment,  # 评论的内容
                "user_name": order.user.name if order.user.name != order.user.mobile else "匿名用户",
                "ctime": order.update_time.strftime("%Y-%d-%m %H:%M:%S")  # 评论时间
            }
